...
{
#pragma HLS array_partition variable=vmap factor=partition_factor BLOCK
...
{
#pragma HLS UNROLL FACTOR=outer_unroll

	for(int x=0;x<KNOB_WINDOW_SIZE_X*3;x++,j++)
	{
#pragma HLS UNROLL FACTOR=inner_unroll1
		if(j==cols-1)
		{
			j=-1;
			nmap[i + x*3] = NAN;
		...
		for(int x = 0; x < KNOB_WINDOW_SIZE_X; x++)
				{
			#pragma HLS UNROLL FACTOR=inner_unroll2
