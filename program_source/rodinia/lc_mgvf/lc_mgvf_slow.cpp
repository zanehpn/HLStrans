...
float lc_mgvf_kernel(float result[GRID_ROWS * GRID_COLS], float imgvf[GRID_ROWS * GRID_COLS], float I[GRID_ROWS * GRID_COLS])
{
    float total_diff = 0.0;

L1:    for (int i = 0; i < GRID_ROWS; i++) {
...
void workload(float result[GRID_ROWS * GRID_COLS], float imgvf[GRID_ROWS * GRID_COLS], float I[GRID_ROWS * GRID_COLS])
{
    ...
    int i;
    float diff = 1.0;
L3:    for (i = 0; i < ITERATION / 2; i++) {
        diff = lc_mgvf_kernel(result, imgvf, I);
        diff = lc_mgvf_kernel(imgvf, result, I);
    }
    return;
