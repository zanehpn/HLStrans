...
static void col_forward(double v[60][60], double u[60][60], double p[60][60], double q[60][60], int i) {
    #pragma HLS INLINE
    for (int j = 1; j < 59; j++) {
...
static void col_backward(double v[60][60], double p[60][60], double q[60][60], int i) {
    #pragma HLS INLINE
    for (int j = 57; j >= 0; j--) {
        #pragma HLS PIPELINE II=1
        const int idx = j + 1;
...
static void column_sweep(double v[60][60], double u[60][60], double p[60][60], double q[60][60]) {
    ...
    COL_TILING: for (int i = 1; i < 59; i++) {
        ...
        col_forward(v, u, p, q, i);
        
        v[59][i] = 1.0;
        col_backward(v, p, q, i);
    }
}

static void row_forward(double u[60][60], double v[60][60], double p[60][60], double q[60][60], int i) {
    #pragma HLS INLINE
    for (int j = 1; j < 59; j++) {
        #pragma HLS PIPELINE II=1
...
static void row_backward(double u[60][60], double p[60][60], double q[60][60], int i) {
    #pragma HLS INLINE
    for (int j = 57; j >= 0; j--) {
        #pragma HLS PIPELINE II=1
        const int idx = j + 1;
...
static void row_sweep(double u[60][60], double v[60][60], double p[60][60], double q[60][60]) {
    ...
    ROW_TILING: for (int i = 1; i < 59; i++) {
        ...
        row_forward(u, v, p, q, i);
        
        u[i][59] = 1.0;
        row_backward(u, p, q, i);
    }
}
...
void adi(int tsteps, int n, double u[60][60], double v[60][60], double p[60][60], double q[60][60]) {
    ...
    #pragma HLS ARRAY_PARTITION variable=p cyclic factor=8 dim=2
