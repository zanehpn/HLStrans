...
void latnrm_optimized(float data[NPOINTS], float outa[NPOINTS], float coefficient[ORDER*2],
                        float internal_state[ORDER+1])
{
    #pragma HLS ARRAY_PARTITION variable=internal_state complete dim=1
    #pragma HLS ARRAY_PARTITION variable=data complete dim=1
    #pragma HLS ARRAY_PARTITION variable=outa complete dim=1
    #pragma HLS ARRAY_PARTITION variable=coefficient complete dim=1
