...
static void aes_init_T(void) {
    VITIS_LOOP_43_1: for (int x = 0; x < 256; x++) {
        uint32_t s = S[x];
...
static void aes256_key_expand(const uint8_t key[32], uint32_t rk[60]) {
    ...
    VITIS_LOOP_60_1: for (int i = 0; i < 8; i++)
        rk[i] = ((uint32_t)key[4*i] << 24)
              | ((uint32_t)key[4*i+1] << 16)
              | ((uint32_t)key[4*i+2] << 8)
              | ((uint32_t)key[4*i+3] );
    uint32_t rcon = 1;
    VITIS_LOOP_66_2: for (int i = 8; i < 60; i++) {
        temp = rk[i-1];
        if (i % 8 == 0) {
...
__attribute__((sdx_kernel("aes_table", 0))) void aes_table(const uint8_t in[16], uint8_t out[16], const uint32_t rk[60]) {
#line 8 "/home/zqy/LLM4CHIP/dataset/pair_slow_fast/New/machsuite/aes_table/hls_template.tcl"
#pragma HLSDIRECTIVE TOP name=aes_table
# 91 "aes_table_fast.c"
    ...
    VITIS_LOOP_105_1: for (int r = 1; r < 14; r++) {
