...
float hotspot_kernel_stencil_core(float temp_top, float temp_left, float temp_right, float temp_bottom, float temp_center, float power_center, float Cap_1, float Rx_1, float Ry_1, float Rz_1) {
...
L11:        for (k = 0; k < PARA_FACTOR; k++) {
            ...
            result[i * PARA_FACTOR + k] = hotspot_kernel_stencil_core(temp_top[k], temp_left[k], temp_right[k], temp_bottom[k], temp_center[k], power_center[k], Cap_1, Rx_1, Ry_1, Rz_1);
        }
...
L14:    float result_inner [TILE_ROWS * GRID_COLS];
L15:    float temp_inner   [(TILE_ROWS + 2) * GRID_COLS];
L16:    float power_inner  [TILE_ROWS * GRID_COLS];
...
L17:    for (i = 0; i < SIM_TIME/2; i++) {
L18:        for (k = 0; k < GRID_ROWS / TILE_ROWS; k++) {
...
L19:        for (k = 0; k < GRID_ROWS / TILE_ROWS; k++) {
