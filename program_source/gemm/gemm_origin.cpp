// Converted from gemm.c to .cpp by convert_and_transform_with_deepseek_auto.py
...
void gemm( TYPE m1[N], TYPE m2[N], TYPE prod[N] ){
    int i, j, k;
#pragma HLS ARRAY_PARTITION variable=prod type=cyclic dim=1 factor=2
#pragma HLS ARRAY_PARTITION variable=m2 type=cyclic dim=1 factor=1
#pragma HLS ARRAY_PARTITION variable=m1 type=cyclic dim=1 factor=1
    int k_col, i_col;
    TYPE mult;
