...
L1:    init_row: for(a_idx=0; a_idx<(ALEN+1); a_idx++){
        M[a_idx] = a_idx * GAP_SCORE;
    }
L2:    init_col: for(b_idx=0; b_idx<(BLEN+1); b_idx++){
        M[b_idx*(ALEN+1)] = b_idx * GAP_SCORE;
    }

    // Matrix filling loop
L3:    fill_out: for(b_idx=1; b_idx<(BLEN+1); b_idx++){
L4:        fill_in: for(a_idx=1; a_idx<(ALEN+1); a_idx++){
    ...
    // Pad the result
L5:    pad_a: for( ; a_str_idx<ALEN+BLEN; a_str_idx++ ) {
      alignedA[a_str_idx] = '_';
    }
L6:    pad_b: for( ; b_str_idx<ALEN+BLEN; b_str_idx++ ) {
...
L7:	int M[(ALEN+1)*(BLEN+1)];
L8:	char ptr[(ALEN+1)*(BLEN+1)];
	int i;
L9:	for (i=0; i<100 /*num_jobs*/; i++) {
