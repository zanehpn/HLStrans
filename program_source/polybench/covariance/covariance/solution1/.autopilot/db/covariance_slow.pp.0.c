# 1 "/home/zqy/LLM4CHIP/dataset/pair_slow_fast/SYN_dataset/polybench/covariance/covariance_slow.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 338 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "/home/zqy/LLM4CHIP/dataset/pair_slow_fast/SYN_dataset/polybench/covariance/covariance_slow.c" 2

__attribute__((sdx_kernel("covariance", 0))) void covariance(int m,int n,double float_n,double data[100][80],double cov[80][80],double mean[80])
{
#line 7 "/home/zqy/LLM4CHIP/dataset/pair_slow_fast/SYN_dataset/polybench/covariance/run_hls.tcl"
#pragma HLSDIRECTIVE TOP name=covariance
# 3 "/home/zqy/LLM4CHIP/dataset/pair_slow_fast/SYN_dataset/polybench/covariance/covariance_slow.c"
  ...
  VITIS_LOOP_11_1: for (j = 0; j < 80; j++) {
    mean[j] = 0.0;

    VITIS_LOOP_14_2: for (i = 0; i < 100; i++) {
      mean[j] += data[i][j];
    }
    mean[j] /= float_n;
  }

  VITIS_LOOP_20_3: for (i = 0; i < 100; i++) {

    VITIS_LOOP_22_4: for (j = 0; j < 80; j++) {
      data[i][j] -= mean[j];
    }
  }

  VITIS_LOOP_27_5: for (i = 0; i < 80; i++) {

    VITIS_LOOP_29_6: for (j = i; j < 80; j++) {
      cov[i][j] = 0.0;

      VITIS_LOOP_32_7: for (k = 0; k < 100; k++) {
