...
#pragma HLS INLINE off
  if (flag) {
L13:    for (int i=0; i<UNROLL_FACTOR; i++) {
...
#pragma HLS INLINE off
  if (flag) {
L14:    for (int i=0; i<UNROLL_FACTOR; i++) {
...
L16:  char seqA_buf_x[UNROLL_FACTOR][ALEN * JOBS_PER_PE];
L17:  char seqA_buf_y[UNROLL_FACTOR][ALEN * JOBS_PER_PE];

L18:  char seqB_buf_x[UNROLL_FACTOR][BLEN * JOBS_PER_PE];
L19:  char seqB_buf_y[UNROLL_FACTOR][BLEN * JOBS_PER_PE];

L20:  char alignedA_buf_x[UNROLL_FACTOR][(ALEN+BLEN) * JOBS_PER_PE];
L21:  char alignedA_buf_y[UNROLL_FACTOR][(ALEN+BLEN) * JOBS_PER_PE];

L22:  char alignedB_buf_x[UNROLL_FACTOR][(ALEN+BLEN) * JOBS_PER_PE];
L23:  char alignedB_buf_y[UNROLL_FACTOR][(ALEN+BLEN) * JOBS_PER_PE];

  int i;
L24:  for (i=0; i<102 /*num_batches+2*/; i++) {
