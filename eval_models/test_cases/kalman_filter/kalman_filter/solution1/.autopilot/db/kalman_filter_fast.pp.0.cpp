# 1 "/home/zqy/LLM4CHIP/C2HLS/eval_models/test_cases/kalman_filter/kalman_filter_fast.cpp"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 375 "<built-in>" 3
# 1 "<command line>" 1
...
# 2 "<built-in>" 2
# 1 "/home/zqy/LLM4CHIP/C2HLS/eval_models/test_cases/kalman_filter/kalman_filter_fast.cpp" 2
...
# 10 "/tools/Xilinx/new/Vitis_HLS/2022.2/common/technology/autopilot/ap_fixed.h" 2
...
# 11 "/tools/Xilinx/new/Vitis_HLS/2022.2/common/technology/autopilot/ap_int.h"
...
# 2 "/home/zqy/LLM4CHIP/C2HLS/eval_models/test_cases/kalman_filter/kalman_filter_fast.cpp" 2
# 1 "/tools/Xilinx/new/Vitis_HLS/2022.2/common/technology/autopilot/ap_int.h" 1
# 3 "/home/zqy/LLM4CHIP/C2HLS/eval_models/test_cases/kalman_filter/kalman_filter_fast.cpp" 2
...
typedef ap_fixed<13 + 6, 13, AP_RND> DTYPE;





typedef ap_uint<256> INTERFACE_WIDTH;
...
extern "C"{
 ...
 __attribute__((sdx_kernel("kalman_filter", 0))) void kalman_filter(INTERFACE_WIDTH in[262144 / (256 / (sizeof(float) * 8))], INTERFACE_WIDTH out[262144 / (256 / (sizeof(float) * 8))]) {
#line 7 "/home/zqy/LLM4CHIP/C2HLS/eval_models/test_cases/kalman_filter/run_hls.tcl"
#pragma HLSDIRECTIVE TOP name=kalman_filter
# 27 "/home/zqy/LLM4CHIP/C2HLS/eval_models/test_cases/kalman_filter/kalman_filter_fast.cpp"

#pragma HLS INTERFACE m_axi port=in bundle=gmem0
#pragma HLS INTERFACE m_axi port=out bundle=gmem1
#pragma HLS INTERFACE s_axilite port=return bundle=control

 static DTYPE in_local[4096 * 64];
#pragma HLS ARRAY_PARTITION variable=in_local complete dim=1
 static DTYPE out_local[4096 * 64];
#pragma HLS ARRAY_PARTITION variable=out_local complete dim=1

 static DTYPE u_hat_arr[64];
#pragma HLS ARRAY_PARTITION variable=u_hat_arr complete dim=1
 static DTYPE p_arr_1[64];
#pragma HLS ARRAY_PARTITION variable=p_arr_1 complete dim=1
 static DTYPE k_arr[64];
#pragma HLS ARRAY_PARTITION variable=k_arr complete dim=1
 static DTYPE calc_temp_arr[64];
#pragma HLS ARRAY_PARTITION variable=calc_temp_arr complete dim=1
 ...
 INTERFACE_WIDTH temp;
  ...
  READ_INPUT: for(int i = 0; i < 262144 / (256 / (sizeof(float) * 8)); i++) {
#pragma HLS PIPELINE II=1
 temp = in[i];
   VITIS_LOOP_53_1: for(int j = 0; j < (256 / (sizeof(float) * 8)); j++) {
    raw_float t;
    t.raw_val = temp.range((j + 1) * (sizeof(float) * 8) - 1, j * (sizeof(float) * 8));
    in_local[counter++] = (DTYPE)t.float_val;
   }
  }


  VITIS_LOOP_61_2: for (int i = 0; i < 64; i++) {
#pragma HLS UNROLL
 u_hat_arr[i] = in_local[i * 4096];
   p_arr_1[i] = 0.5;
   out_local[i * 4096] = u_hat_arr[i];
  }


  KALMAN_LOOP_T: for (int t = 1; t < 4096; t++) {
#pragma HLS LOOP_TRIPCOUNT min=4095 max=4095
 KALMAN_LOOP_I: for (int i = 0; i < 64; i++) {
#pragma HLS UNROLL factor=4
 calc_temp_arr[i] = p_arr_1[i] + (DTYPE)0.01;
    k_arr[i] = calc_temp_arr[i] / (p_arr_1[i] + (DTYPE)1.01);
    u_hat_arr[i] = u_hat_arr[i] + k_arr[i] * (in_local[i * 4096 + t] - u_hat_arr[i]);
    p_arr_1[i] = ((DTYPE)1 - k_arr[i]) * calc_temp_arr[i];
    out_local[i * 4096 + t] = u_hat_arr[i];
   }
  }


  counter = 0;
  WRITE_OUTPUT: for(int i = 0; i < 262144 / (256 / (sizeof(float) * 8)); i++) {
#pragma HLS PIPELINE II=1
 temp = 0;
   VITIS_LOOP_86_3: for(int j = 0; j < (256 / (sizeof(float) * 8)); j++) {
    raw_float t;
    t.float_val = (float)out_local[counter++];
    temp.range((j + 1) * (sizeof(float) * 8) - 1, j * (sizeof(float) * 8)) = t.raw_val;
   }
   out[i] = temp;
  }
 }
