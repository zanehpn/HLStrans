...
typedef struct {
    ...
    XCompute_near_uio_map maps[ MAX_UIO_MAPS ];
} XCompute_near_uio_info;

/***************** Variable Definitions **************************************/
static XCompute_near_uio_info uio_info;
...
static int uio_info_read_name(XCompute_near_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XCompute_near_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XCompute_near_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XCompute_near_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XCompute_near_Initialize(XCompute_near *InstancePtr, const char* InstanceName) {
	XCompute_near_uio_info *InfoPtr = &uio_info;
