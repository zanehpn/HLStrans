...
void kmeans_clustering(float points[N_POINTS][N_FEATURES], float centroids[N_CLUSTERS][N_FEATURES], int assignments[N_POINTS]) {
    float* new_centroids = new float[N_CLUSTERS * N_FEATURES](); // zero-initialized
    int* counts = new int[N_CLUSTERS](); // zero-initialized
    ...
    for (int i = 0; i < N_POINTS; i++) {
        ...
        float* nc = &new_centroids[closest_centroid * N_FEATURES];
        for (int k = 0; k < N_FEATURES; k++) {
            nc[k] += points[i][k];
        }
    }
    ...
    for (int j = 0; j < N_CLUSTERS; j++) {
        if (counts[j] > 0) {
            ...
            for (int k = 0; k < N_FEATURES; k++) {
                centroids[j][k] = new_centroids[j * N_FEATURES + k] * inv;
            }
        }
    }
