...
#pragma HLS array_partition variable=win_buf.win block factor = tmpdim dim=0
#pragma HLS array_partition variable=templ block factor = tmpsize
...
#pragma HLS UNROLL FACTOR=UNROLL_LOOP1
			for (n = 0; n < tmpdim; n++) {
		...
		for (l = 0; l < tmpdim; l++) {
#pragma HLS UNROLL FACTOR=UNROLL_LOOP2
	    ...
	    int absl = 0;
	    for (y = 0; y < tmpdim; y++) {
#pragma HLS UNROLL FACTOR=UNROLL_LOOP3
	        for (z = 0; z < tmpdim; z++) {
...
#pragma HLS UNROLL FACTOR=UNROLL_LOOP4
				for(o = 0; o < indim; o++) {
