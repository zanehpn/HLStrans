...
# 11 "./stencil.h" 2
# 24 "./stencil.h"
__attribute__((sdx_kernel("stencil3d", 0))) void stencil3d( int32_t C[2], int32_t orig[(16 * 32 * 32)], int32_t sol[(16 * 32 * 32)] );
...
struct bench_args_t {
  int32_t C[2];
  int32_t orig[(16 * 32 * 32)];
  int32_t sol[(16 * 32 * 32)];
};
# 9 "stencil.c" 2

__attribute__((sdx_kernel("stencil3d", 0))) void stencil3d(int32_t C[2], int32_t orig[(16 * 32 * 32)], int32_t sol[(16 * 32 * 32)]) {
#line 8 "/home/zqy/LLM4CHIP/HLSFactory/hlsfactory/hls_dataset_sources/machsuite/stencil3D/hls_template.tcl"
...
# 10 "stencil.c"
    ...
    height_bound_col : for(j=0; j<32; j++) {
        height_bound_row : for(k=0; k<16; k++) {
            sol[((k)+16*((j)+32*(0)))] = orig[((k)+16*((j)+32*(0)))];
            sol[((k)+16*((j)+32*(32 -1)))] = orig[((k)+16*((j)+32*(32 -1)))];
        }
    }
    col_bound_height : for(i=1; i<32 -1; i++) {
        col_bound_row : for(k=0; k<16; k++) {
            sol[((k)+16*((0)+32*(i)))] = orig[((k)+16*((0)+32*(i)))];
            sol[((k)+16*((32 -1)+32*(i)))] = orig[((k)+16*((32 -1)+32*(i)))];
        }
    }
    row_bound_height : for(i=1; i<32 -1; i++) {
        row_bound_col : for(j=1; j<32 -1; j++) {
    ...
    loop_height : for(i = 1; i < 32 - 1; i++){
        loop_col : for(j = 1; j < 32 - 1; j++){
            loop_row : for(k = 1; k < 16 - 1; k++){
                sum0 = orig[((k)+16*((j)+32*(i)))];
                sum1 = orig[((k)+16*((j)+32*(i + 1)))] +
                       orig[((k)+16*((j)+32*(i - 1)))] +
                       orig[((k)+16*((j + 1)+32*(i)))] +
                       orig[((k)+16*((j - 1)+32*(i)))] +
                       orig[((k + 1)+16*((j)+32*(i)))] +
