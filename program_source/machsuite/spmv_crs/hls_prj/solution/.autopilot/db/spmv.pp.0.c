...
__attribute__((sdx_kernel("spmv", 0))) void spmv(double val[1666], int32_t cols[1666], int32_t rowDelimiters[494 +1], double vec[494], double out[494]){
#line 8 "/home/zqy/LLM4CHIP/dataset/pair_slow_fast/New/machsuite/spmv_crs/hls_template.tcl"
...
#pragma HLS loop_tripcount min=1 max=494 avg=494
 sum = 0; Si = 0;
        int tmp_begin = rowDelimiters[i];
        int tmp_end = rowDelimiters[i+1];
        spmv_2 : for (j = tmp_begin; j < tmp_end; j++){
#pragma HLS loop_tripcount min=0 max=1666 avg=1666/494
 Si = val[j] * vec[cols[j]];
            sum = sum + Si;
        }
        out[i] = sum;
