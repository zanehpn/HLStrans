...
void pll(float input_signal[N], float output_signal[N], float ref_frequency, float loop_bandwidth, float damping_factor) {
    ...
    float* phase_error   = new float[N]();
    float* vco_phase     = new float[N]();
    float* vco_frequency = new float[N]();
    float* integrator    = new float[N]();
    ...
    for (int i = 0; i < N; i++) {
        ...
        // Output signal
        output_signal[i] = std::sin(vco_phase[i]);
    }

    delete[] phase_error;
    delete[] vco_phase;
    delete[] vco_frequency;
    delete[] integrator;
}
