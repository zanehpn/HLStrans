...
L1:    int M[(ALEN+1)*(BLEN+1)];
L2:    char ptr[(ALEN+1)*(BLEN+1)];
...
L3:    init_row: for(a_idx=0; a_idx<(ALEN+1); a_idx++){
        M[a_idx] = a_idx * GAP_SCORE;
    }
L4:    init_col: for(b_idx=0; b_idx<(BLEN+1); b_idx++){
        M[b_idx*(ALEN+1)] = b_idx * GAP_SCORE;
    }

    // Matrix filling loop
L5:    fill_out: for(b_idx=1; b_idx<(BLEN+1); b_idx++){
L6:        fill_in: for(a_idx=1; a_idx<(ALEN+1); a_idx++){
    ...
    // Pad the result
L7:    pad_a: for( ; a_str_idx<ALEN+BLEN; a_str_idx++ ) {
      alignedA[a_str_idx] = '_';
    }
L8:    pad_b: for( ; b_str_idx<ALEN+BLEN; b_str_idx++ ) {
...
L9:	char seqA_buf[ALEN * JOBS_PER_BATCH];
L10:	char seqB_buf[BLEN * JOBS_PER_BATCH];
L11:	char alignedA_buf[(ALEN+BLEN) * JOBS_PER_BATCH];
L12:	char alignedB_buf[(ALEN+BLEN) * JOBS_PER_BATCH];

	int num_batches = num_jobs / JOBS_PER_BATCH;

	int i, j, k;
L13:	for (i=0; i<100 /*num_batches*/; i++) {
	    // step 1: copy data in
