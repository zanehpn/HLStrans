...
__attribute__((sdx_kernel("spmv_ellpack", 0))) void spmv_ellpack(double nzval[1024*1024], int32_t cols[1024*1024], double vec[1024], double out[1024]);



struct bench_args_t {
  double nzval[1024*1024];
  int32_t cols[1024*1024];
  double vec[1024];
  double out[1024];
};
# 7 "spmv_ellpack.c" 2
...
__attribute__((sdx_kernel("spmv_ellpack", 0))) void spmv_ellpack(double nzval[1024*1024], int32_t cols[1024*1024], double vec[1024], double out[1024])
{
#line 8 "/home/zqy/LLM4CHIP/dataset/pair_slow_fast/New/machsuite/spmv_ellpack/hls_template.tcl"
#pragma HLSDIRECTIVE TOP name=spmv_ellpack
# 9 "spmv_ellpack.c"

    int i, j;
    double Si;

    ellpack_1 : for (i=0; i<1024; i++) {
        double sum = out[i];
        ellpack_2 : for (j=0; j<1024; j++) {
                Si = nzval[j + i*1024] * vec[cols[j + i*1024]];
                sum += Si;
        }
        out[i] = sum;
    }
